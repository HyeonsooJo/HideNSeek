...
def likelihood_ratio_filter(node_pairs, modified_adjacency, original_adjacency, d_min, threshold=0.004, undirected=True):
    ...
    """
    if not torch.is_tensor(modified_adjacency):
        if sp.issparse(modified_adjacency): modified_adjacency = np.array(modified_adjacency.todense())
        modified_adjacency = torch.tensor(modified_adjacency, dtype=torch.int64)
    if not torch.is_tensor(original_adjacency):
        if sp.issparse(original_adjacency): original_adjacency = np.array(original_adjacency.todense())
        original_adjacency = torch.tensor(original_adjacency, dtype=torch.int64)
...
def likelihood_ratio_filter_mettack(node_pairs, modified_adjacency, original_adjacency, d_min, threshold=0.004, undirected=True):
    """
    Filter the input node pairs based on the likelihood ratio test proposed by Zügner et al. 2018, see
    https://dl.acm.org/citation.cfm?id=3220078. In essence, for each node pair return 1 if adding/removing the edge
    between the two nodes does not violate the unnoticeability constraint, and return 0 otherwise. Assumes unweighted
